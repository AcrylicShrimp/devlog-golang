...
func (pq *PostQuery) QueryThumbnail() *PostThumbnailQuery {
	query := &PostThumbnailQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryImages() *PostImageQuery {
	query := &PostImageQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryVideos() *PostVideoQuery {
	query := &PostVideoQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) WithThumbnail(opts ...func(*PostThumbnailQuery)) *PostQuery {
	query := &PostThumbnailQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withThumbnail = query
	return pq
}
...
func (pq *PostQuery) WithImages(opts ...func(*PostImageQuery)) *PostQuery {
	query := &PostImageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) WithVideos(opts ...func(*PostVideoQuery)) *PostQuery {
	query := &PostVideoQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context) ([]*Post, error) {
	...
	if query := pq.withThumbnail; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.PostThumbnail(func(s *sql.Selector) {
			s.Where(sql.InValues(post.ThumbnailColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.post_thumbnail
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "post_thumbnail" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "post_thumbnail" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := pq.withImages; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Images = []*PostImage{}
		}
		query.withFKs = true
		query.Where(predicate.PostImage(func(s *sql.Selector) {
			s.Where(sql.InValues(post.ImagesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.post_images
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "post_images" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "post_images" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := pq.withVideos; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Videos = []*PostVideo{}
		}
		query.withFKs = true
		query.Where(predicate.PostVideo(func(s *sql.Selector) {
			s.Where(sql.InValues(post.VideosColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.post_videos
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "post_videos" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
